  ...
  public static void enterFluidEvents(
    ...
    if (!(event.getEntityLiving() instanceof Player)) {
      if (event.getEntityLiving() instanceof Zombie
          || event.getEntityLiving() instanceof Skeleton) {
        ...
        if (isInHerbalBath) {
          ...
          if (event.getEntityLiving().tickCount % damageIntervalTicks == 0) {
            event.getEntityLiving().hurt(DamageSource.MAGIC, damagePerSecond);
          }
        }
      }
      return;
    ...
    boolean isInHerbalBath =
        CustomFluidHandler.isPlayerInHerbalBathBlock((Player) event.getEntityLiving());

    if (event.getEntityLiving() instanceof ServerPlayer player) {
      CompoundTag playerData = player.getPersistentData();

      if (isInHerbalBath) {
        HotWaterEvents.handleAdvancement(enteredCountTriggerNumber, enteredNumberInHerbalBath, herbalBathStayedTime, hasEnteredHerbalBath, herbalBathAdvancementId, player, playerData);
