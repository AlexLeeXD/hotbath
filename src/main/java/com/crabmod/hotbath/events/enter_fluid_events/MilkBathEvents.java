...
public class MilkBathEvents {
  ...
  public static void enterFluidEvents(
    ...
    if (event.getEntityLiving() instanceof ServerPlayerEntity) {
      ...
      boolean isInMilkBath =
          CustomFluidHandler.isPlayerInMilkBathBlock(player);
      ...
      if (isInMilkBath) {
          ...
          if (enteredCount >= enteredCountTriggerNumber) {
            Advancement advancement =
                Objects.requireNonNull(player.getServer())
                    ...
                    .getAdvancement(
                        Objects.requireNonNull(ResourceLocation.tryCreate(milkBathAdvancementId)));
        ...
        if (playerData.getInt(milkBathStayedTime) >= stayedEffectTriggerTime * TICK_NUMBER) {
          EffectRemovalHandler.removeNegativeEffectsExceptUnluck(
              player);
        }
      } else {
