...
public abstract class AbstractHotBathBlock extends FlowingFluidBlock {
  ...
  public static void enterHotBathEvents(
      ...
      if (playerData.getInt(playerDataId) >= effectParameter.stayTime * TICK_NUMBER) {
        serverPlayerEntity.addPotionEffect(
            new EffectInstance(
                ...
                TICK_NUMBER * effectParameter.effectTime,
                effectParameter.amplifierIn,
                false,
                false,
                true));
      }
    ...
    if (attributeParameter != null) {
      if (playerData.getInt(playerDataId) >= attributeParameter.stayTime * TICK_NUMBER) {
        ...
        EffectChangeUtil.applyAttributeModifier(
            serverPlayerEntity, attributeParameter.attribute, attributeParameter.modifier);
      }
    }
  }

  public static void enterHotBathAdancements(
      ServerPlayerEntity serverPlayerEntity,
      Integer enteredCount,
      String enteredCountDataId,
      String AdvancementId) {
    CompoundNBT playerData = serverPlayerEntity.getPersistentData();

    if (playerData.getInt(enteredCountDataId) >= enteredCount) {
      ...
      if (advancement != null) {
        ...
        playerData.putInt(enteredCountDataId, 0);
      }
    }
  }
  ...
  public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
    ...
    if (entityIn instanceof ServerPlayerEntity) {
      ...
      if (lastTick + 5 <= worldIn.getGameTime()) {
        ...
        LOGGER.info("worldIn.getGameTime(): " + worldIn.getGameTime());
      ...
      specialEffect(
          serverPlayerEntity,
          playerData.getInt(stayTimeDataId),
          playerData.getInt(enteredCountDataId));
      enterHotBathAdancements(serverPlayerEntity, enteredCount, enteredCountDataId, advancementId);

      LOGGER.info("enteredCount: " + playerData.getInt(enteredCountDataId));
    }
  }
